...
TYPE(A_VOL_GQP),ALLOCATABLE,DIMENSION(:)::QP_ARRAY !CELL NUM, QP NUM

TYPE::A_U_CENTRE					!DATA TYPE THAT HOLDS THE CELL CENTER VALUES FOR THE EQUATIONS TO BE SOLVED
...
TYPE::A_mass_matrix					!DATA TYPE THAT HOLDS THE DG_mass_matrix
	REAL,ALLOCATABLE,DIMENSION(:,:)::VAL	
END TYPE A_mass_matrix

TYPE::A_FACE_D
	INTEGER,ALLOCATABLE,DIMENSION(:)::Q_MAPL,QMAPD
END TYPE A_FACE_D

TYPE(A_U_CENTRE),ALLOCATABLE,DIMENSION(:)::U_C 	!1-D ARRAY FOR TYPE FOR SOLUTION OF MEAN FLOW EQUATIONS
TYPE(A_U_CENTRE),ALLOCATABLE,DIMENSION(:)::U_cx,U_CS,U_CW 	!1-D ARRAY FOR TYPE FOR SOLUTION OF MEAN FLOW EQUATIONS
TYPE(A_U_CENTRE),ALLOCATABLE,DIMENSION(:)::U_CT 	!1-D ARRAY FOR TYPE FOR SOLUTION OF TURBULENT EQUATIONS 
TYPE(A_mass_matrix),ALLOCATABLE,DIMENSION(:)::m_1 	!1-D ARRAY FOR TYPE FOR SOLUTION OF TURBULENT EQUATIONS


TYPE::A_U_EXACT					!DATA TYPE THAT HOLDS THE CELL CENTER VALUES OF THE EXACT SOLUTION FOR LINEAR ADVECTION OF EULER EQUATIONS IF AN EXACT SOLUTION EXISTS
	REAL,ALLOCATABLE,DIMENSION(:,:)::VAL	!VALUES OF THE EXACT SOLUTION
	REAL,ALLOCATABLE,DIMENSION(:,:,:)::VALDG
END TYPE A_U_EXACT
TYPE(A_U_EXACT),ALLOCATABLE,DIMENSION(:)::U_E	!1-D ARRAY FOR TYPE FOR EXACT SOLUTION OF MEAN FLOW EQUATIONS


TYPE::A_SUMFLUXES					!DATA TYPE FOR THE FLUXES CONTRIBUTION
...
END TYPE A_SUMFLUXES
TYPE(A_SUMFLUXES),ALLOCATABLE,DIMENSION(:)::SUMCONFLUX	!CONVECTIVE FLUXES MEAN FLOW EQUATIONS
TYPE(A_SUMFLUXES),ALLOCATABLE,DIMENSION(:)::SUMVISCFLUX	!VISCOUS FLUXES MEAN FLOW EQUATIONS
TYPE(A_SUMFLUXES),ALLOCATABLE,DIMENSION(:)::SOURCETERM	!SOURCETERM
TYPE(A_SUMFLUXES),ALLOCATABLE,DIMENSION(:)::SUMCONFLUXT	!CONVECTIVE FLUXES ADDITIONAL EQUATIONS
TYPE(A_SUMFLUXES),ALLOCATABLE,DIMENSION(:)::SUMVISCFLUXT  !VISCOUS FLUXES ADDITIONAL EQUATIONS
TYPE(A_SUMFLUXES),ALLOCATABLE,DIMENSION(:)::RHS		!RIGHT HAND SIDE OF THE MEAN FLOW EQUATIONS (SUM OF ALL FLUXES CONTRIBUTIONS)
TYPE(A_SUMFLUXES),ALLOCATABLE,DIMENSION(:)::RHST		!RIGHT HAND SIDE OF THE ADDITIONAL FLOW EQUATIONS (SUM OF ALL FLUXES CONTRIBUTIONS)


TYPE A_INTEGRALBASIS !INTEGRAL BASIS FUNCTIONS
	REAL,allocatable,dimension(:)::VALUE,VALUEc !VALUES FOR THE HIGH AND LOWER-ORDER POLYNOMIALS RESPECTIVELY
END TYPE A_INTEGRALBASIS
TYPE(A_INTEGRALBASIS),ALLOCATABLE,DIMENSION(:)::INTEG_BASIS,integ_basis_dg
...
TYPE(A_LOCAL_RECON3),ALLOCATABLE,DIMENSION(:)::ILOCAL_RECON3
TYPE(A_NEIXX),ALLOCATABLE,DIMENSION(:)::NEIX1,NEIX2,NEIX3,NEIX4,NEIX5
TYPE(A_LOCAL_RECON3),ALLOCATABLE,DIMENSION(:)::ILOCAL_RECON4,ILOCAL_RECON5,ilocal_recon6

TYPE A_EXCHANGE_CORD
...
END TYPE A_EXCHANGE_CORD

TYPE(A_EXCHANGE_CORD),ALLOCATABLE,DIMENSION(:)::IEXCORDR  !RECEIVING DATA TYPE     
TYPE(A_EXCHANGE_CORD),ALLOCATABLE,DIMENSION(:)::IEXCORDS  !SENDING DATA TYPE


TYPE A_EXCHANGE_SOLHI
	INTEGER::PROCID    !ID OF PROCESSOR
...
END TYPE A_EXCHANGE_SOLHI

TYPE(A_EXCHANGE_SOLHI),ALLOCATABLE,DIMENSION(:)::IEXSOLHIR,iexsolhird    !RECEIVING DATA TYPE FOR HALO CELLS OF STENCILS
TYPE(A_EXCHANGE_SOLHI),ALLOCATABLE,DIMENSION(:)::IEXSOLHIS,iexsolhisd    !SENDING DATA TYPE FOR HALO CELLS OF STENCILS



TYPE A_EXCHANGE_BOUNDHI    
	INTEGER::PROCID   !ID OF PROCESSOR
	INTEGER::IAVT     !NUMBER OF PROCESSORS THAT WE NEED TO RECEIVE/OR SEND DATA
	INTEGER::FAST
	INTEGER::HOWMANY  !HOW MANY ELEMENTS ARE NEEDED
...
END TYPE A_EXCHANGE_BOUNDHI

TYPE(A_EXCHANGE_BOUNDHI),ALLOCATABLE,DIMENSION(:)::IEXBOUNDHIR !RECEIVING DATA TYPE FOR BOUNDARY EXTRAPOLATED VALUES AT GAUSSIAN QUADRATURE POINTS OF INTER-PROCESSOR BOUNDARIES
TYPE(A_EXCHANGE_BOUNDHI),ALLOCATABLE,DIMENSION(:)::IEXBOUNDHIS !SENDING DATA TYPE FOR BOUNDARY EXTRAPOLATED VALUES AT GAUSSIAN QUADRATURE POINTS OF INTER-PROCESSOR BOUNDARIES
TYPE(A_EXCHANGE_BOUNDHI),ALLOCATABLE,DIMENSION(:)::IEXBOUNDHIRi !RECEIVING DATA TYPE FOR BOUNDARY EXTRAPOLATED VALUES AT GAUSSIAN QUADRATURE POINTS OF INTER-PROCESSOR BOUNDARIES FOR IMPLICIT TIME STEPPING
TYPE(A_EXCHANGE_BOUNDHI),ALLOCATABLE,DIMENSION(:)::IEXBOUNDHIsi !SENDING DATA TYPE FOR BOUNDARY EXTRAPOLATED VALUES AT GAUSSIAN QUADRATURE POINTS OF INTER-PROCESSOR BOUNDARIES FOR IMPLICIT TIME STEPPING
TYPE(A_EXCHANGE_BOUNDHI),ALLOCATABLE,DIMENSION(:)::IEXBOUNDHIRR  !RECEIVING DATA TYPE FOR BOUNDARY EXTRAPOLATED VALUES AT GAUSSIAN QUADRATURE POINTS OF INTER-PROCESSOR BOUNDARIES FOR IMPLICIT TIME STEPPING
TYPE(A_EXCHANGE_BOUNDHI),ALLOCATABLE,DIMENSION(:)::IEXBOUNDHISS  !SENDING DATA TYPE FOR BOUNDARY EXTRAPOLATED VALUES AT GAUSSIAN QUADRATURE POINTS OF INTER-PROCESSOR BOUNDARIES FOR IMPLICIT TIME STEPPING



!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
TYPE::A_NODE_NE	!NAME OF TYPE FOR THE SET OF NODES 
...
END TYPE A_NODE_NE
TYPE::A_NODE_LIT	!NAME OF TYPE FOR THE SET OF NODES 
...
TYPE(A_NODE_NE),ALLOCATABLE,DIMENSION(:)::INODEN,INODER,INODER4
TYPE(A_NODE_LIT),ALLOCATABLE,DIMENSION(:)::INODER2



TYPE::A_ELEM_SHAPE	!NAME OF TYPE FOR THE SET OF NODES 
	INTEGER::ISHAPE	!integer indicating the shape of each element
END TYPE A_ELEM_SHAPE 
...
TYPE(A_EXCHANGE),ALLOCATABLE,DIMENSION(:)::IEXCHANGER !receive
TYPE(A_EXCHANGE),ALLOCATABLE,DIMENSION(:)::IEXCHANGES !send
TYPE(A_EXCHANGE),ALLOCATABLE,DIMENSION(:)::IEXCHANGER1
TYPE(A_EXCHANGE),ALLOCATABLE,DIMENSION(:)::IEXCHANGES1
...
END TYPE A_SOLEXCHANGE

TYPE(A_SOLEXCHANGE),ALLOCATABLE,DIMENSION(:)::SOLCHANGER  !RECEIVES

TYPE(A_SOLEXCHANGE),ALLOCATABLE,DIMENSION(:)::SOLCHANGES  !SENDS
...
TYPE(A_RECEX),ALLOCATABLE,DIMENSION(:)::IRECEXR		!RECEIVE ELEMENTS DUE TO STENCILS
TYPE(A_RECEX),ALLOCATABLE,DIMENSION(:)::IRECEXR1		!RECEIVE ELEMENTS DUE TO STENCILS
TYPE(A_RECEX),ALLOCATABLE,DIMENSION(:)::irecexrg		!RECEIVE ELEMENTS DUE TO STENCILS
TYPE(A_RECEX),ALLOCATABLE,DIMENSION(:)::irecexsg		!SEND ELEMENTS DUE TO STENCILS
TYPE(A_RECEX),ALLOCATABLE,DIMENSION(:)::IRECEXS1		!SEND ELEMENTS DUE TO STENCILS
TYPE(A_RECEX),ALLOCATABLE,DIMENSION(:)::IRECEXS		!SEND ELEMENTS DUE TO STENCIL


!>NAME OF TYPE FOR THE SET OF ELEMENTS
TYPE::A_ELEMENT_NUMBER
	...
	TYPE(A_FACE_D),ALLOCATABLE,DIMENSION(:)::Q_FACE  !QUADRATURE FACE
...
TYPE(A_ELEMENT_NUMBER),ALLOCATABLE,DIMENSION(:,:)::IELEM  !ELEMENT NUMBER
TYPE(A_FACES),ALLOCATABLE,DIMENSION(:,:)::IFACE


TYPE::A_BOUND_NUMBER	!NAME OF TYPE FOR THE SET OF ELEMENTS
...
END TYPE A_BOUND_NUMBER

TYPE(A_BOUND_NUMBER),ALLOCATABLE,DIMENSION(:,:)::IBOUND	  !1-D ARRAY FOR BOUNDARY CONDITION TYPE

TYPE::A_CONNX	!NAME OF TYPE FOR THE SET OF ELEMENTS
...
TYPE(A_CONNX),ALLOCATABLE,DIMENSION(:)::ICONR
TYPE(A_CONNX),ALLOCATABLE,DIMENSION(:)::ICONS
TYPE(A_CONNX),ALLOCATABLE,DIMENSION(:)::ICONRPA
TYPE(A_CONNX),ALLOCATABLE,DIMENSION(:)::ICONRPM
TYPE(A_CONNX),ALLOCATABLE,DIMENSION(:)::ICONSPO
TYPE(A_CONNX),ALLOCATABLE,DIMENSION(:)::ICONRPF



TYPE::A_NODE_NUMBER	!NAME OF TYPE FOR THE SET OF NODES 
...
END TYPE A_NODE_NUMBER 


TYPE(A_NODE_NUMBER),ALLOCATABLE,DIMENSION(:,:)::INODE	  !1-D ARRAY FOR POINTER TYPE FOR NODES

TYPE(A_NODE_NUMBER)::ITEMP				  !TEMPORARY POINTER NODE ITERATION

	TYPE A_LISTSTENCILS
	INTEGER::PROCID
	INTEGER,ALLOCATABLE,DIMENSION(:)::LISTSARRAY
	END TYPE A_LISTSTENCILS
	...
	TYPE::A_elementglobal
  INTEGER::ELEMENTGLID,NodeID1,NodeID2,NodeID3,NodeID4,NodeID5,NodeID6,NodeID7,NodeID8!,vweight!,vsize
END TYPE A_elementglobal

TYPE::A_NODE_NUMBER1	!NAME OF TYPE FOR THE SET OF NODES 
	INTEGER::NODEN	!IDENTIFICATION NUMBER THAT CAN BE USED AS A POINTER INSIDE AN ARRAY
	REAL::X		!COORDINATES IN X AXIS
	REAL::Y		!COORDINATES IN Y AXIS
	Real::z
END TYPE A_NODE_NUMBER1
TYPE::A_ELEMENT_NUMBER1	!NAME OF TYPE FOR THE SET OF ELEMENTS
    INTEGER::IEINDEX
    INTEGER::IECOUNTER
    Integer:: IFACE
...
END TYPE A_ELEMENT_NUMBER1
TYPE::A_BOUNDARY_NUMBER1	!NAME OF TYPE FOR THE SET OF ELEMENTS
    INTEGER::IBINDEX
    INTEGER::IBCOUNTER
    Integer:: IBTYPE
    Integer,allocatable,dimension(:):: NDB
    INTEGER ::IBShape ! 1 line ! 3 triangle ! 4 quad
END TYPE A_BOUNDARY_NUMBER1
TYPE::A_FACE_Number	!NAME OF TYPE FOR THE SET OF ELEMENTS
    INTEGER::IFINDEX
    INTEGER::IFCOUNTER
